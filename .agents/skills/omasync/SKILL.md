...
- Keep EyrWSL native-Herdr-only: the native binary and `hdw` remain, and the `herdr/` package stows Omarchy's `config/herdr/config.toml` (compare with `git show v4.0.0:config/herdr/config.toml`; only the header and `onboarding = false` differ); open it with `herdr`. No copied `hdl`/`hdlm`/`hsl`/`hds`, Omarchy AI shortcuts, `h`/`t` aliases, or local tmux package/config/recipes. Never import desktop launch helpers during sync; the SSH reconnect helper's remote-tmux context remains valid. Keep Windows Terminal `Alt+Enter` pass-through for Omarchy's Herdr map, without treating retirement as settings-deployment approval.
