...
- Source retirement does not uninstall an existing host package or kill sessions. Actual WSL retirement follows the guarded [host procedure](docs/handoff.md#3-deploy-eyrwsl), preserving sessions, real directories and user state; inspect installed package ownership and reverse dependencies before exact package-removal approval. Do not force dependencies, use blanket `-Rns`/orphan cleanup, or overwrite Terminal settings for this migration.
